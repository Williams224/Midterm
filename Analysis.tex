...
\end{equation}
  ...
  The parameter $\alpha$ describes the cut off where the function changes from a gaussian to a power law. A positive value of $\alpha$ gives a low side tail and a negative value gives a high side tail.

  The signal shape in MC was found to have non-gaussian tails on both sides of the distribution.  Therefore, a double crystal ball function was used with opposite side tails by restricting the two $\alpha$ parameters to have opposite signs.  An extended unbinned maximum likelihood fit was used, for which the likelihood function was given by,
  \begin{equation}
    L=e^{-N_{C1}-N_{C2}}\prod_i^{D}N_{C1}C1(x_i,\alpha_1,n_1,\bar{x}_1,\sigma_1)+N_{C2}C2(x_i,\alpha_2,n_2,\bar{x}_2,\sigma_2)
  \end{equation}

where N_{c} is the number of signal events in each of the crystal ball functions.
